mainApp.controller('storeViewController', ['$rootScope','lodash','$scope','$location','$window','$timeout','SweetAlert','moment','adminService',
    function ($rootScope,lodash, $scope, $location, $window, $timeout,SweetAlert,moment, adminService) {
        ...
        $scope.model.getPayouts = function () {
            adminService.fetchData('/payouts/index?query=store', function (resp) {
        ...
        $scope.model.viewDetails = function (payout) {
            ...
            adminService.fetchData(
                '/payouts/items?type=store&id='+payout.id,
                function(resp){
